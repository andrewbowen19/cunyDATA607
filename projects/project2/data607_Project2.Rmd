...
## Dataset 3
Using this [gun violence dataset](https://www.kaggle.com/datasets/jameslko/gun-violence-data?resource=download) from Kaggle. Since the original file is too big to store on GitHub, I took the first 10000 rows. Will need to tidy up some columns before we can aanlyze
...
```{r}
head(gv$participant_age, 20)
```

Let's also break up our `participant_age` column so it's a bit cleaner
```{r}
gv <- gv  %>%
