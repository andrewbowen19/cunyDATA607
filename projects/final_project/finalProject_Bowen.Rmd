...
sample_aq <- lookupAQ(40.7219, -73.9788, 2011)
...
```{r}
nyc_df <- nyc_aq
nyc_aq <- nyc_df %>% pivot_wider(., names_from = name, values_from=data_value)
# %>%
#                 group_by(borough, year) %>%
#                 rename("ozone" = "Ozone (O3)", "sulfur_o2" = "Sulfur Dioxide (SO2)") %>%
#                 # filter(!is.na(ozone) & !is.na(sulfur_o2)) %>%
#                 summarise(mean_ozone = mean(ozone, na.rm=TRUE), mean_sulfur_o2 = mean(sulfur_o2, na.rm=TRUE))
```
